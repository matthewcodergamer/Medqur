...
import 'prescription_order_detail_page.dart';
import 'profile_page_v2.dart';
import 'scan_page_v2.dart';

/// Role-aware clinical shell with V0.14 prescription review/amendment and
/// dispensing workflow. The class name remains V3 for source compatibility.
...
class _ClinicalShellV3State extends State<ClinicalShellV3> {
  ...
  Future<void> _pharmacyPatientsChanged() async {
  ...
  Future<void> _newEncounter() async {
    ...
    if (!_doctor &&
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content:
              Text('Patient registration is limited to authorized intake staff.'),
        ),
      );
      return;
    }
  ...
  void _openMedicationOrder(Patient patient, MedicationOrder order) {
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => PrescriptionOrderDetailPage(
          staff: widget.staff,
          patient: patient,
          facility: widget.facility,
          order: order,
          onAmended: (updated) async {
            var index = patient.medications.indexWhere(
              (item) => identical(item, order),
            );
            if (index < 0 && order.orderId != null) {
              index = patient.medications
                  .indexWhere((item) => item.orderId == order.orderId);
            }
            if (index >= 0) {
              patient.medications[index] = updated;
            } else {
              patient.medications.add(updated);
            }
            patient.status = PatientStatus.treatment;
            await widget.onPatientsChanged();
            if (mounted) setState(() {});
      ...
      3 => _doctor
          ? DoctorOrdersHubPage(
              ...
              diagnosticOrders: _orders
                  ...
                  .toList(),
              onOpenPatient: _openPatient,
              onOpenMedicationOrder: _openMedicationOrder,
          ...
          : _pharmacy
              ? PharmacyPage(
                  ...
                  facility: widget.facility,
                  patients: widget.patients,
                  onPatientsChanged: _pharmacyPatientsChanged,
                )
