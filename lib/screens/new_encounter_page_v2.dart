...
class _NewEncounterPageV2State extends State<NewEncounterPageV2> {
  ...
  String _shortId() => (DateTime.now().millisecondsSinceEpoch % 1000000).toString().padLeft(6, '0');
  ...
  Patient _patient() {
    ...
    return Patient(
      ...
      timeline: [
        ...
        if (capturedCredential != null) '${_timeNow()} — Identity credential scanned; authoritative verification still pending',
        '${_timeNow()} — ${triageLabel(triage)} triage recorded',
      ],
      medications: [],
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(backgroundColor: Colors.white, surfaceTintColor: Colors.white, title: const Text('New encounter', style: TextStyle(fontWeight: FontWeight.w800))),
      body: AnimatedSwitcher(duration: const Duration(milliseconds: 240), child: stage == 0 ? _registration() : _wristband()),
    );
  }

  Widget _registration() {
    return ListView(
      key: const ValueKey('registration-v2'),
      ...
      children: [
        ...
        if (identity == 'NIDS / NIC') ...[
          OutlinedButton.icon(onPressed: _scanIdentity, icon: const Icon(Icons.badge_outlined), label: Text(capturedCredential == null ? 'Scan NIDS / NIC' : 'Credential captured • scan again')),
          const SizedBox(height: 10),
          const Text('Scanning captures the credential only. NIRA identity verification is not connected to this public prototype.', style: TextStyle(color: Color(0xFF748297), fontSize: 12)),
        ...
        if (identity != 'Emergency / unknown') ...[
          ...
          const SoftCard(highlighted: true, child: Row(children: [Icon(Icons.emergency_rounded, color: medqurAmber), SizedBox(width: 12), Expanded(child: Text('Emergency care can start immediately; identity can be reconciled later.'))])),
        ...
        FilledButton.icon(
          ...
          icon: Icon(identity == 'Emergency / unknown' ? Icons.emergency_rounded : Icons.arrow_forward_rounded),
          label: Text(identity == 'Emergency / unknown' ? 'Create emergency encounter' : 'Continue to wristband'),
        ),
      ],
    );
  }
  ...
  Widget _wristband() {
    ...
    return ListView(
      key: const ValueKey('wristband-v2'),
      padding: const EdgeInsets.fromLTRB(20, 28, 20, 32),
      children: [
        const Center(child: Icon(Icons.check_circle_rounded, color: medqurGreen, size: 52)),
        const SizedBox(height: 10),
        Text('Encounter ready', textAlign: TextAlign.center, style: Theme.of(context).textTheme.headlineSmall),
        const SizedBox(height: 22),
        Center(
          child: ConstrainedBox(
            ...
            child: SoftCard(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                const Row(children: [MedqurLogo(width: 118), Spacer(), Text('ENCOUNTER WRISTBAND', style: TextStyle(color: Color(0xFF8793A4), fontSize: 10, fontWeight: FontWeight.w900, letterSpacing: 1))]),
                const SizedBox(height: 16),
                Row(children: [
                  ...
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    const SizedBox(height: 10),
                    StatusPill(label: triageLabel(patient.triage), color: triageColor(patient.triage)),
                  ])),
                ]),
                const SizedBox(height: 14),
                const Divider(color: medqurLine),
                const SizedBox(height: 8),
                const Text('The QR contains only an opaque encounter token, not diagnoses or confidential clinical data.', style: TextStyle(color: Color(0xFF748297), fontSize: 12)),
              ]),
            ),
          ),
        ),
        const SizedBox(height: 20),
        FilledButton.icon(onPressed: () => Navigator.of(context).pop(patient), icon: const Icon(Icons.print_rounded), label: const Text('Print wristband & add to queue')),
        const SizedBox(height: 8),
