...
/// V0.14 prescription composer.
///
/// Changes from the earlier composer:
/// - prescription ink is consistently blue; the blue/black toggle is removed
/// - frequency runs from 1x through 20x
/// - route choices are broad and alphabetized
/// - common directions/warnings live in a searchable picker instead of a wall
///   of chips
/// - an existing prescription can be reopened and amended; the old signed
///   revision is kept in [PrescriptionRecordStore]
...
class _PrescriptionComposerPageState extends State<PrescriptionComposerPage> {
  ...
  void _seedExisting() {
    ...
    _duration.text = record?.duration ?? '';
    ...
    _doseUnit = switch (unitToken) {
      ...
      'ul' || 'µl' || 'µl' => MedicationDoseUnit.microlitre,
  ...
  Future<void> _pickCommonInstruction() async {
    var query = '';
    ...
    if (selected != null) _appendPhrase(selected);
  }
  ...
  Future<void> _signAndPreview() async {
    ...
    try {
      // Creating a new order is safe against the current backend. For an
      // amendment to a backend-backed order, keep the signed revision locally
      // rather than silently creating a second active server order. A central
      // amendment endpoint can later supersede the server revision atomically.
      ...
      body: SafeArea(
        child: MedqurPage(
          children: [
            ...
            MedqurPageHeader(
              eyebrow: widget.isAmendment ? 'Signed order amendment' : 'Doctor order',
              title: widget.isAmendment ? 'Update prescription' : 'New prescription',
                ...
                children: [
                  ...
                  Align(
                    ...
                    child: TextButton.icon(
                      onPressed: _pickCommonInstruction,
                      icon: const Icon(Icons.add_rounded),
                      label: const Text('Add common instruction'),
                    ),
                  ),
                  const SizedBox(height: 7),
